...
import {
  ...
  Image as ImageIcon, AlertTriangle, Zap, Check, Eye, Save, ArrowLeft
} from 'lucide-react';
...
export default function EditTaskPage() {
    ...
    const fetchTask = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setTaskData({ ...data, id: docSnap.id } as TaskData);
          
          let pickupTimeStr = '';
          if (data.pickupTime) {
            const pt = typeof data.pickupTime === 'string' ? new Date(data.pickupTime) : (data.pickupTime as any).toDate ? (data.pickupTime as any).toDate() : new Date(data.pickupTime);
            pickupTimeStr = format(pt, "yyyy-MM-dd'T'HH:mm");
          }
          
          let dropoffTimeStr = '';
          if (data.deadline || data.dropoffTime) {
            const dt = data.dropoffTime || data.deadline;
            const dtObj = typeof dt === 'string' ? new Date(dt) : (dt as any).toDate ? (dt as any).toDate() : new Date(dt!);
            dropoffTimeStr = format(dtObj, "yyyy-MM-dd'T'HH:mm");
          }
          ...
          setFormData({
            ...
            bounty: data.bounty?.toString() || '0',
            currency: data.currency || 'INR',
            weight: data.weight?.toString() || '0.5',
            priorityLevel: data.priorityLevel || 'standard',
            parcelType: data.parcelType || 'other',
  ...
  const handleSave = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!user || !taskId) return toast.error('Sign in required');
  ...
  const dummyPreviewData: TaskData = {
    ...
    createdByName: taskData?.createdByName || profile?.displayName || 'Your Profile',
    reactionCount: taskData?.reactionCount || 0,
    deadline: formData.dropoffTime || null
  } as unknown as TaskData;

  if (loading || fetching) return <div className="h-screen flex flex-col items-center justify-center"><Loader2 className="w-12 h-12 text-blue-500 animate-spin mb-4" /><p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Accessing Intel Matrix...</p></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pt-24 pb-32 overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none opacity-40">
        <div className="absolute top-0 -left-20 w-[500px] h-[500px] bg-indigo-100 rounded-full blur-[120px]" />
        <div className="absolute bottom-0 -right-20 w-[500px] h-[500px] bg-emerald-100 rounded-full blur-[120px]" />
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 relative z-10">
        ...
        <div className="mb-8">
           <button 
             onClick={() => router.back()} 
             className="flex items-center gap-2 px-4 py-2 rounded-xl text-slate-500 hover:text-slate-900 bg-white/50 border border-slate-200 backdrop-blur-sm transition-all hover:bg-white cursor-pointer font-bold text-sm shadow-sm"
           >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 text-blue-600 text-xs font-black uppercase tracking-wider mb-3">
              <Zap className="w-3.5 h-3.5" /> Premium Task Modificator
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">
              Modify <span className="text-blue-600">Mission</span>
            </h1>
            <p className="text-slate-500 font-medium mt-2">Target: #{taskId.slice(0, 8)}</p>
          </div>
          
          <div className="flex gap-3">
             <button 
                onClick={() => setShowPreview(!showPreview)}
                className={`flex items-center gap-2 px-5 h-12 rounded-2xl font-bold transition-all cursor-pointer ${
          ...
          <div className="lg:col-span-12 space-y-8">
            ...
            <Section icon={<Package className="text-emerald-500" />} title="Parcel Specification">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                       {PARCEL_SIZES.map(s => (
                         <button 
                            ...
                            className={`p-3 rounded-xl border-2 transition-all flex flex-col items-center cursor-pointer ${
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 mt-6 p-4 bg-slate-50/80 rounded-2xl border border-slate-100">
                    ...
                    <button type="button" className="flex items-center gap-2 text-blue-600 hover:text-blue-700 transition-colors py-1 px-2 rounded-lg hover:bg-blue-50 w-fit cursor-pointer">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {['today', 'tomorrow', 'week'].map(t => (
                        <button 
                          ...
                          className="px-4 py-2 rounded-xl bg-slate-100 text-slate-500 text-[10px] font-black uppercase hover:bg-slate-200 transition-all hover:scale-105 active:scale-95 cursor-pointer"
                        >
                          {t}
                        </button>
                      ))}
               ...
               <div className="grid md:grid-cols-2 gap-8 items-start">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       {PRIORITY_OPTIONS.map(opt => (
                         <button 
                            ...
                            className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center cursor-pointer ${
                    ...
                    <button 
                      ...
                      className={`w-full mt-4 p-4 rounded-2xl border-2 transition-all flex items-center justify-between cursor-pointer ${
            ...
            <div className="bg-white p-8 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 text-[10px] font-black text-emerald-600 uppercase tracking-widest">
                       <Sparkles className="w-3 h-3" /> Mission Modification Override
                    </div>
                  </div>
               </div>
               ...
               <Button 
                onClick={() => handleSave()}
                disabled={submitting}
                className="h-16 px-12 rounded-full bg-blue-600 hover:bg-blue-700 text-white font-black tracking-wide shadow-lg shadow-blue-200 transition-all active:scale-95 text-lg cursor-pointer"
               >
                  {submitting ? <Loader2 className="animate-spin mr-3" /> : <Save className="w-5 h-5 mr-3" />}
                  COMMIT CHANGES
               </Button>
            </div>

            <div className="flex items-center justify-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                <ShieldAlert className="w-4 h-4" /> Modifications logged & monitored for authenticity
            </div>
